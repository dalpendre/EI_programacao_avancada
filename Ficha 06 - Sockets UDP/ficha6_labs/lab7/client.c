...
int main(int argc, char *argv[]){
    ...
    if(connect(udp_client_socket, (struct sockaddr *) &udp_server_endpoint, udp_server_endpoint_length))
    {
    ...
    fflush(stdout);
	if((udp_sent_bytes = send(udp_client_socket, &number, sizeof(number), 0)) == -1)
    ...
    fflush(stdout);
	if((udp_read_bytes = recv(udp_client_socket, buffer, sizeof(buffer), 0)) == -1)
