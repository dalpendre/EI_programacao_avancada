...
int main(int argc, char *argv[])
{
    ...
    FILE *fptr = fopen(args_info.filename_arg, "r");    //Ficheiro a extrair conteúdo
    if(fptr == NULL)
    {
        ERROR(1, "\n\tFile does not exist\n");
    }

    ssize_t filesize = fseek(fptr, 0L, SEEK_END);
    filesize = ftell(fptr);
    ...
    do
    {
        file_caracther = fgetc(fptr);
        file_content[counter] = file_caracther;
        counter++;
    }while(file_caracther != EOF);
    ...
    char filename[MAX_FILENAME];  //Mensagem recebida
                
    // TCP IPv4: "recv" do cliente (bloqueante)
    printf("à espera de dados do cliente... "); 
    fflush(stdout);
    if((tcp_read_bytes = recv(tcp_client_socket, &filename, sizeof(filename), 0)) == -1)
        ERROR(57, "Can't recv from client");
    printf("ok.  (%d bytes sent)\n", (int)tcp_read_bytes);

    //Cópia do conteúdo do ficheiro para o ficheiro do cliente
